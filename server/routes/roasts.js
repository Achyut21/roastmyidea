...
router.get('/ideas/:id/roasts', async (req, res) => {
  ...
  const roasts = await db.collection('roasts').find({ ideaId, deleted: false }).sort({ upvoteCount: -1, createdAt: -1 }).toArray();
  ...
  res.json({ roasts: roasts.map((r) => ({ ...r, authorDisplayName: authorMap[r.authorId.toString()] || 'Unknown' })) });
});
...
router.post('/ideas/:id/roasts', requireAuth, async (req, res) => {
  ...
  if (!content || content.trim().length < 10 || content.trim().length > 500) return res.status(400).json({ error: 'Roast must be between 10 and 500 characters' });
...
router.put('/roasts/:id', requireAuth, async (req, res) => {
  ...
  if (roast.authorId.toString() !== req.user._id.toString()) return res.status(403).json({ error: 'Not your roast' });
  const { content } = req.body;
  if (!content || content.trim().length < 10 || content.trim().length > 500) return res.status(400).json({ error: 'Roast must be between 10 and 500 characters' });
  await db.collection('roasts').updateOne({ _id: roastId }, { $set: { content: content.trim() } });
...
router.delete('/roasts/:id', requireAuth, async (req, res) => {
  ...
  await db.collection('ideas').updateOne({ _id: roast.ideaId }, { $inc: { roastCount: -1 } });
...
router.post('/roasts/:id/upvote', requireAuth, async (req, res) => {
  ...
  if (alreadyVoted) {
    await db.collection('roasts').updateOne({ _id: roastId }, { $pull: { upvotedBy: userId }, $inc: { upvoteCount: -1 } });
  } else {
    await db.collection('roasts').updateOne({ _id: roastId }, { $addToSet: { upvotedBy: userId }, $inc: { upvoteCount: 1 } });
  }
  const updated = await db.collection('roasts').findOne({ _id: roastId });
