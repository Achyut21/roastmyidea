...
router.get('/roasts/:id/defenses', async (req, res) => {
  ...
  res.json({
    defenses: defenses.map((d) => ({
      ...d,
      authorDisplayName: authorMap[d.authorId.toString()] || 'Unknown',
    })),
  });
});
...
router.post('/roasts/:id/defenses', requireAuth, async (req, res) => {
  ...
  if (!content || content.trim().length < 10 || content.trim().length > 500)
    return res
      .status(400)
      .json({ error: 'Defense must be between 10 and 500 characters' });
  const authorId = req.user._id;
  if (roast.authorId.toString() === authorId.toString())
    return res.status(403).json({ error: "You can't defend against your own roast" });
  const existing = await db
    .collection('defenses')
    .findOne({ roastId, authorId, deleted: false });
  if (existing)
    return res.status(400).json({ error: 'You already defended against this roast' });
  const hasRoasted = await db
    .collection('roasts')
    .findOne({ ideaId: roast.ideaId, authorId, deleted: false });
  if (hasRoasted)
    return res.status(400).json({ error: "You already roasted this idea, you can't switch sides" });
  ...
  await db
    .collection('roasts')
    .updateOne({ _id: roastId }, { $inc: { defenseCount: 1 } });
...
router.put('/defenses/:id', requireAuth, async (req, res) => {
  ...
  const defense = await db
    .collection('defenses')
    .findOne({ _id: defenseId, deleted: false });
  if (!defense) return res.status(404).json({ error: 'Defense not found' });
  if (defense.authorId.toString() !== req.user._id.toString())
    return res.status(403).json({ error: 'Not your defense' });
  const { content } = req.body;
  if (!content || content.trim().length < 10 || content.trim().length > 500)
    return res
      .status(400)
  ...
  await db
    .collection('defenses')
    .updateOne({ _id: defenseId }, { $set: { content: content.trim() } });
...
router.delete('/defenses/:id', requireAuth, async (req, res) => {
  ...
  await db
    .collection('defenses')
    .updateOne({ _id: defenseId }, { $set: { deleted: true } });
  await db
    .collection('ideas')
    .updateOne({ _id: defense.ideaId }, { $inc: { defenseCount: -1 } });
  await db
    .collection('roasts')
    .updateOne({ _id: defense.roastId }, { $inc: { defenseCount: -1 } });
...
router.post('/defenses/:id/upvote', requireAuth, async (req, res) => {
  ...
  const alreadyVoted = defense.upvotedBy.some(
